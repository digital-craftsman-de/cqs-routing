...
This might be useful when the flow of a command handler should be stopped but no error must be shown to the user. For example when a command to trigger a change of an email address is executed but the email address hasn't changed. No confirmation email must be send out and no data must be stored. But the user must also not get an error because the email address itself is valid.
